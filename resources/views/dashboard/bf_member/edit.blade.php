<form id="edit_bf_member_form" data = "{{$bf_member->slug}}">
  @csrf
  <div class="modal-header">
    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
    <h4 class="modal-title">{{$bf_member->lastname}}, {{$bf_member->firstname}} {{substr($bf_member->middlename, 0, 1) }}.</h4>
  </div>
    ...
    <div class="row">
      ...
      <div class="col-md-8">
        ...
        <div class="input-group block_farm except has-success">
          <input autocomplete="off" name="block_farm" placeholder="Block Farm the farmer belongs to" type="text" class="form-control block_farm_search" value="{{$bf_member->blockFarm->block_farm_name}}" readonly="">
            ...
            <dl>
              <div class="row">
                <div class="col-md-5">
                   ...
                   <dd>{{$bf_member->blockFarm->block_farm_name }}</dd>
                </div>
                <div class="col-md-3">
                  <dt>Mill District</dt>
                  <dd>{{$bf_member->blockFarm->millDistrict->mill_district }}</dd>
                </div>
                <div class="col-md-4">
                  <dt>Enrolee</dt>
                  <dd>{{$bf_member->blockFarm->enrolee_name }}</dd>
                </div>
              </div>
            </dl>
          </div>
          ...
          <table class="table table-bordered table-striped table-hover" id="family_table_edit" style="width: 100% !important; font-size: 14px;">
            ...
            <tbody>
              @if(!empty($bf_member->familyMembers))
                @foreach($bf_member->familyMembers as $family_member)
                <tr id="member-{{$family_member->slug}}">
                  <td>
                    @php
                      $data = [
                  ...
                  <td>
                    {{$family_member->lastname}}, {{$family_member->firstname}} {{substr($family_member->middlename, 0, 1) }}.
                  </td>
                  <td>{{$family_member->sex }}</td>
                  <td>{{date("m/d/Y",strtotime($family_member->bday)) }}</td>
                  <td>{{$family_member->educ_att }}</td>
                  <td>{{$family_member->civil_status }} | {{$family_member->eco_status }}</td>
                  <td>
                    <div class="btn-group">
                      <button type="button" data="member-{{$family_member->slug}}" class="btn btn-default btn-sm edit_family_btn_edit" data-toggle="modal" data-target="#edit_family_modal_edit" title="" data-placement="top" data-original-title="Edit">
...
{{--    
    <button class="get_datas" type="button">GET</button>
    <button class="final" type="button">FINALE</button> 
--}}
  </div>
...
<script type="text/javascript">
  chosen_bf_edit = "{{$bf_member->blockFarm->slug}}";
